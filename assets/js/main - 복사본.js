...
// --- 정보 카드 인라인 수정 ---
function showEditInfoCard(id) {
    const row = document.getElementById(`info-card-row-${id}`);
    row.querySelectorAll('.view-mode').forEach(el => el.classList.add('hidden'));
    row.querySelectorAll('.edit-mode').forEach(el => el.classList.remove('hidden'));
}

function hideEditInfoCard(id) {
    const row = document.getElementById(`info-card-row-${id}`);
    row.querySelectorAll('.view-mode').forEach(el => el.classList.remove('hidden'));
    row.querySelectorAll('.edit-mode').forEach(el => el.classList.add('hidden'));
}

async function saveInfoCard(id) {
    const row = document.getElementById(`info-card-row-${id}`);
    ...
    formData.append('content', inputs[1].value.trim());
    formData.append('url', inputs[2].value.trim());

    try {
        const res = await fetch('actions/edit_info_card.php', { method: 'POST', body: new URLSearchParams(formData) });
        const data = await res.json();

        if (res.ok && !data.error) {
            const viewSpans = row.querySelectorAll('.view-mode');
            
            // viewSpans[0] 은 제목 컬럼의 span
            viewSpans[0].textContent = data.title;
            
            // viewSpans[1] 은 내용 컬럼의 span (개행 문자를 <br> 태그로 변환)
            viewSpans[1].innerHTML = data.content.replace(/\n/g, '<br>');
            
            // viewSpans[2] 는 URL 컬럼의 span
            const link = viewSpans[2].querySelector('a');
            link.href = data.url;
            link.textContent = data.url;
            
            hideEditInfoCard(id); // 수정 모드 닫기
        } else { 
            alert('카드 저장 실패: ' + (data.error || '알 수 없는 오류')); 
        }
    } catch (error) { 
        console.error('Save info card error:', error);
        alert('저장 중 오류가 발생했습니다.'); 
    }
}
